...
public class LoopClosureSoftConstraintController implements Controller
{
    ...
    * </p>
    *
    * @param name                            the name of the constraint, {@code YoVariable}s will be
    *                                        created using this name.
    * @param transformToParentJoint          the pose of the constraint with respect to the parent
    *                                        joint.
    * @param transformToSuccessorParentJoint the pose of the constraint with respect to the parent
    *                                        joint of the associated successor. Note that the
    *                                        successor's parent joint is expected to be different from
    *                                        this constraint's parent joint.
    * @param constraintForceSubSpace         defines the linear part of the sub-space in which the
    *                                        constraint is to be enforced.
    * @param constraintMomentSubSpace        defines the angular part of the sub-space in which the
    *                                        constraint is to be enforced.
    */
   public LoopClosureSoftConstraintController(String name,
                                              RigidBodyTransformReadOnly transformToParentJoint,
                                              RigidBodyTransformReadOnly transformToSuccessorParentJoint,
                                              ...
                                              Matrix3DReadOnly constraintMomentSubSpace)
   {
      ...
      constraintDefinitionA = new ExternalWrenchPointDefinition(name + "A", transformToParentJoint);
      constraintDefinitionB = new ExternalWrenchPointDefinition(name + "B", transformToSuccessorParentJoint);
   }
      ...
      if (isFirstUpdate)
      {
         if (proportionalGains.containsNaN() || derivativeGains.containsNaN())
            throw new IllegalArgumentException("The gains for the loop closure constraint: " + name
                  + " have not been configured. If created from description, see: " + LoopClosureSoftConstraintController.class.getSimpleName());
         if (TupleTools.isTupleZero(proportionalGains, 0.0) || TupleTools.isTupleZero(derivativeGains, 0.0))
            LogTools.warn("The gains for the loop closure constraint: " + name + " have not been configured. If created from description, see: "
                  + LoopClosureSoftConstraintController.class.getSimpleName());
