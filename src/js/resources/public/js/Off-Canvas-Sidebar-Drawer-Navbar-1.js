...
There are a number of specific css classes you need to be aware of, but for the most part, you can use this simply by dropping it into your project.
It all starts at the top level with a standard Navbar component.
If you select that, you will see a "data-open-drawer" attribute that you can set to 1 to show the nav bar for development purposes.
While open, customize it for your needs. As is, there is a brand heading and close button on the top.
Then a dynamic height middle nav that will scroll within itself if it becomes too long.
Lastly there is a fixed bottom nav which can be entirely removed without causing any issues if you don't need it.
There is also a predefined "open drawer" button that can be easily adjusted or removed without causing any issues.
The Navbar should be set to be fluid and have a position of "Fixed to top".
It also requires my added class of "off-canvas".
Things you will probably want or need to change in the css file (off-canvas-sidebar.css): the default opening button settings and responsive drawer widths.
Most of that is near the bottom of the file.
Honestly, that's about it... unless you run into trouble with some z-index stuff... but my usage of that is very light.
The CSS file was created to do only what was needed to enforce the drawer's functionality.
It doesn't do much of anything for visual styles like coloring and such.
You can use the standard bootstrap studio controls to change the colors and everything else mostly.
Though, I would recommend caution when messing with flex settings or responsive display stuff... but if you mess it up in the editor,
going back to default should fix it and reset things to what the css file specifies.

In my development version, the CSS file is organized into 5 sections.
Section 1 and 2 are responsible for the core settings to make the drawer work accordingly.
Section 3 is where you can change the settings of the default "open drawer" button - I call it a "drawer knob".
Section 4 is for setting the drawer width per responsive breakpoint.
And Section 5 is an advanced section focused on expanding on the default functionality of this component.
It provides CSS so that with a few extra divs and classes, you can change how the drawer "slides" onto your page - it can be over top of the site, push the site, slide the site.
Also in that section, I had instruction on how to use more included CSS with just one added div to fade your site's content while the drawer is open.
I will provide all of that documentation here in a moment.

There are 2 included JavaScript files with this component: swipe.js and off-canvas-sidebar.js (this file).
The swipe.js file contains a simple and clean implementation of pure javascript swipe event detection (mostly for mobile devices).
I did not write that code - as stated at the top of that file.
I did write the off-canvas-sidebar.js (this) file to provide simple interaction support for opening and closing your drawers.
It allows you to place anything on your site anywhere with attributes of data-open="drawer", data-dismiss="drawer", and/or data-toggle="drawer" to automatically add click/touch support to those items to open, close, or toggle the drawer, respectively.
...
*** If you want the drawer to be open on load, you will need to be sure to add the "open" class to the navbar (and you should also add "open" to site wrapper if you are using the dom and css from the slide or push styles detailed in section 5 notes).
If you are using the fade feature, you will also need to add the class "drawer-open" to the body tag.

*** If you want the drawer on the right side, you need to set the "data-right-drawer" attribute on the Navbar to 1 or add the class "right-drawer" to the Navbar.
In this current version, the Right Drawers don't work with slide and push styles. Sorry for any inconvenience.
