...
public class SimpleCrossFourBarURDFRobot
{
   public static void main(String[] args) throws JAXBException
   {
      System.out.println(new Vector2D(0.2, 0.2).norm());

      InputStream is = SimpleCrossFourBarURDFRobot.class.getClassLoader().getResourceAsStream("urdf/SimpleCrossFourBarRobot.urdf");
      ...
      CrossFourBarJointReadOnly joint = robot.getAllJoints()
                                             .stream()
                                             .filter(j -> j instanceof CrossFourBarJointReadOnly)
                                             .map(CrossFourBarJointReadOnly.class::cast)
                                             .findFirst()
                                             .get();
   ...
   public static List<YoGraphicDefinition> frameAfterJointGraphics(CrossFourBarJointReadOnly joint)
   {
      List<YoGraphicDefinition> yoGraphics = new ArrayList<>();

      for (RevoluteJointReadOnly subJoint : Arrays.asList(joint.getJointA(), joint.getJointB(), joint.getJointC(), joint.getJointD()))
      {
   ...
   public static List<YoGraphicDefinition> bodyFixedFrameGraphics(CrossFourBarJointReadOnly joint)
   {
      List<YoGraphicDefinition> yoGraphics = new ArrayList<>();
      ...
      List<MovingReferenceFrame> frames = Arrays.asList(joint.getPredecessor().getBodyFixedFrame(),
                                                        ...
                                                        joint.getBodyDA().getBodyFixedFrame(),
                                                        joint.getBodyBC().getBodyFixedFrame());
